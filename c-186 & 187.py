...
firebase = firebase.FirebaseApplication("https://kwitter-833c7-default-rtdb.firebaseio.com/", None)
...
def enterRoom():
    global username
    global your_code
    global your_friendscode
    global message_text
    global message_entry
    username=username_entry.get()
    your_code=your_code_entry.get()
    your_friendscode=friends_code_entry.get()
...
def getData():
    ...
    get_your_data=firebase.gat('/',your_code)
    print(get_your_data)
    bytes_string=bytes.fromhex(get_your_data)
    original=decrypt("AIM",bytes_string)
    print("Original data "+original)
    final_message=original.decode("utf-8")
    print(final_message)
    message_text.insert(END,final_message+"\n")
    
    get_friends_data=firebase.get('/',your_friendscode)
    print("Data: "+get_friends_data)
    if(get_friends_data != None):
        bytes_string=bytes.fromhex(get_friends_data)
        original=decrypt("AIM",bytes_string)
        final_message=original.decode("utf-8")
